...
export interface ImockDB {
	 ...
	 *
	 */
	putFixture(dbName: string, tableName: string, values: Record<string, string>[]): Promise<void[]>;
	 ...
	 *
	 */
	put(dbName: string, tableName: string, value: Record<string, any>): Promise<number>;
	 ...
	 *
	 */
	putBulk(dbName: string, tableName: string, values: Record<string, string>[]): Promise<void[]>;
...
export class mockDB implements ImockDB {
	...
	public async putFixture(dbName: string, tableName: string, values: Record<string, string>[]) {
	...
	public async put(dbName: string, tableName: string, value: Record<string, any>) {
		const db = await openDB(dbName, 1);
		const transaction = db.transaction(tableName, 'readwrite');
		const store = transaction.objectStore(tableName);
		const result = !this.freezeState ? await store.put(value) : (await store.count()) + 1;
	...
	public async putBulk(dbName: string, tableName: string, values: Record<string, string>[]) {
		const db = await openDB(dbName, 1);
		const transaction = db.transaction(tableName, 'readwrite');
		const store = transaction.objectStore(tableName);
		for (const value of values) {
